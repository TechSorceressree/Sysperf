...
csv_file = os.path.join(data_folder, "system_metrics.csv")
...
def collect_metrics():
    ...
    with open(csv_file, mode="w", newline="") as file:
        ...
        writer.writerow(
            ["Process ID", "Process Name", "Timestamp", "CPU Usage (%)", "Memory Usage (%)", "Disk Usage (%)", "Anomaly"]
        )
        ...
        while timestamp < end:
            ...
            for proc in psutil.process_iter(['pid', 'name', 'cpu_percent', 'memory_percent']):
                ...
                writer.writerow([process_id, process_name, timestamp, cpu_usage, mem_usage, disk_usage, anomaly])
...
def estimate_disk_usage(process_id):
    try:
        ...
        return (total_size / (1024 * 1024))  # Convert bytes to megabytes for disk usage estimation
    except (psutil.NoSuchProcess, psutil.AccessDenied, psutil.ZombieProcess):
        return 0  # Return 0 if the process doesn't exist or if there are permission issues
